...
class Cfe_Queue_Adapter_Activemq extends Zend_Queue_Adapter_AdapterAbstract
{
    ...
    public function setPersistence($persistence) {
        $this->_persistence = ($persistence == true);
    }
    ...
    public function __construct($options, Zend_Queue $queue = null)
    {
        ...
        $this->_client->send($connect);
        if(!$this->_client->canRead()) {
            require_once 'Zend/Queue/Exception.php';
            throw new Zend_Queue_Exception("Unable to authenticate to '".$options['scheme'].'://'.$options['host'].':'.$options['port']."' Timeout");
        }
                ...
                for ($i = 0; $i < $maxMessages; $i++) {
                    ...
                    if($response === false) {
                        ...
                        switch ($response->getCommand()) {
                            case 'MESSAGE':
                                $datum = array(
                                    'timestamp'  => strtotime($response->getHeader('timestamp')),
                                    'message_id' => $response->getHeader('message-id'),
                                    'handle'     => $response->getHeader('message-id'),
                                    'redelivered'=> ($response->getHeader('redelivered')==='true'),
